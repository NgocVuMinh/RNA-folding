...
def train_objective_function(structure_files, atom_type="C3'", mode="histogram", bin_size=1.0, min_dist=3.0, max_dist=20.0):
    ...
    for filepath in structure_files:
        ...
        interactions = get_all_distances(structure, atom_name=atom_type, min_distance=min_dist, max_distance=max_dist)
...
def save_scores(scores_dict, output_dir="potentials"):
    ...
    for pair, dist_values in pair_raw.items():
        ...
        if dist_values.size < 5: 
            # Not enough points for a KDE? Return a flat penalty or skip.
            print(f"  Warning: Insufficient observations for {pair} (N={dist_values.size})")
            # Create a flat "ignore" or "penalty" profile
            scores = np.zeros_like(eval_points) # Or set to high energy
        else:
            print(f"  {pair}: fitting KDE with {dist_values.size} observations...")

            # 4. Fit Pair KDE
            # CRITICAL: Use a scalar bandwidth factor (e.g., 0.2 - 0.5) for RNA pairs
            # to capture sharp atomic interactions. Default is usually too wide.
            try:
                pair_kde = gaussian_kde(dist_values, bw_method=0.3) 
                obs_density = pair_kde(eval_points)
                obs_density = np.maximum(obs_density, epsilon)

                # 5. Compute Energy: -kT * ln(P_obs / P_ref)
                # Since we didn't re-normalize, these are true density ratios.
                scores = -np.log(obs_density / ref_density)
                
                # Soft clamping is safer than hard clipping for gradients
                scores = np.clip(scores, -8.0, 10.0)
                
            except np.linalg.LinAlgError:
                print(f"  Error: Singular matrix for {pair}. Data likely collinear (variance=0).")
                scores = np.full_like(eval_points, 10.0)
