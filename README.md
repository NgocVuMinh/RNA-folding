...
An all-atom version of the statistical potential has been implemented in the `all_atom_version/` folder. Unlike the default implementation which uses a coarse-grained model (C3' atoms only), this version considers interactions between **all atoms** of the RNA residues.

### Key Changes
- **Granularity**: Calculates distances for every atom pair (e.g., `A_N1` to `U_O2`) instead of just residue centers.
- **Dependencies**: The Kernel Density Estimation (KDE) has been switched from R (`rpy2`) to Python (`scipy`) to improve compatibility and ease of installation on Windows.
- **Performance**: Training takes significantly longer due to the massive increase in the number of interaction pairs (dozens of atom types per residue vs 1 C3' atom).

### Usage

To train the all-atom potential, navigate to the specific folder and run the training script:

```bash
cd all_atom_version
python train.py -d ../data/cif -f cif -m histogram -o profiles/all_atom_v1
```
